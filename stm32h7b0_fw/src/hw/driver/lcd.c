...
static const uint8_t pin_3_3  = 0;
static const uint8_t pin_1_8  = 1;
static const uint8_t pin_cs   = 2;
static const uint8_t pin_pd8  = 3;
...
static uint16_t __attribute__((section(".lcd"))) __attribute__((aligned(64))) lcd_buffer[HW_LCD_WIDTH * HW_LCD_HEIGHT];
...
{
  gpioPinWrite(pin_cs,  _DEF_LOW);
  delay(1);
  spiTransfer16(spi_ch, reg);
...
void lcdSetup(void)
{
  ...
  delay(1);
  gpioPinWrite(pin_1_8, _DEF_HIGH);
  delay(10);


  gpioPinWrite(pin_pd8, _DEF_HIGH);
  delay(2);
  gpioPinWrite(pin_pd8, _DEF_LOW);
  delay(2);
  gpioPinWrite(pin_pd8, _DEF_HIGH);
